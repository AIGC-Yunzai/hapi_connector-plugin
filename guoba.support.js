...
export function supportGuoba() {
  ...
  const helpSchemas = [
    helpDivider('必备指令'),
    helpItem('help1', '说明', '#hapi', '所有 #hapi 指令仅对云崽主人权限生效'),
    helpItem('help2', '查看帮助', '#hapi help / #hapi帮助',),
    helpItem('update', '更新插件', '#hapi更新'),
    helpDivider('会话与对话'),
    helpItem('list', '查看 session 列表', '#hapi list [all]', '显示当前聊天 / 全部 session 列表'),
    ...
    helpItem('quick', '快捷发送', '> 内容 / >{N} 内容 / > 上传附件3张 [内容]', '快捷发送到[当前/第N个] session ；使用参数 上传附件n张 时将等待用户发送文件/图片/视频'),
    helpDivider('权限审批'),
    ...
    helpItem('abort', '中断 session', '#hapi abort [目标]', '无 [目标] 参数时为当前 session'),
    helpItem('archive', '归档当前 session', '#hapi archive [目标]', '无 [目标] 参数时为当前 session'),
    helpItem('resume', '恢复 inactive session', '#hapi resume [目标]', '无 [目标] 参数时为当前 session'),
    helpItem('rename', '重命名当前 session', '#hapi rename <新标题>'),
    helpItem('delete', '删除 session', '#hapi delete [目标]', '无 [目标] 参数时为当前 session'),
    ...
    helpDivider('文件操作'),
    helpItem('files', '浏览远端目录', '#hapi files [路径]  /  #hapi files -l [路径]', '参数 -l 同时显示大小'),
    ...
    helpItem('bind1', '默认通知窗口', '#hapi bind [claude|codex|gemini|opencode|all]', '设置指定 flavor 或全部默认通知窗口'),
    ...
    configInfo: {
      schemas: [
        ...
        },
        {
          ...
          bottomHelpMessage: '设置 Markdown 图片输出时的主题；自动：06:00-17:59 使用浅色，其余时间使用深色',
        },
        {
